...
export default function Carrosel({ chamados = [] }) {
  ...
  useEffect(() => {
    ...
    if (nome) {
      ...
      const nomeExibidoCalculado = `${partes[0]?.charAt(0).toUpperCase() + partes[0]?.slice(1).toLowerCase()} ${partes[partes.length - 1]
        ? partes[partes.length - 1].charAt(0).toUpperCase() +
  ...
  return (
    <>
      {chamados.map((chamado, index) => {
        ...
        <div key={index} >
          <div className={`card mt-2 ${isConcluido ? 'card-user-desativado' : 'card-user'} row  borderColorPrioridade-${chamado.grau_prioridade}-user`}
            type="button"
            ...
            <div className="col-12 col-sm-3 col-md-3 align-items-center justify-content-center d-flex mt-2 mt-sm-0">
              <div className={`prioridade-user-${chamado.grau_prioridade} align-items-center justify-content-center d-flex`}>
